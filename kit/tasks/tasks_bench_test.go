...
func BenchmarkSingleTask(b *testing.B) {
	task := NewTask(func(c *Context, input int) (int, error) {
		return input * 2, nil
	})
...
func BenchmarkParallelIndependentTasks(b *testing.B) {
	...
	task3 := NewTask(func(c *Context, input int) (int, error) {
		return input * 4, nil
	})
...
func BenchmarkHighContention(b *testing.B) {
	...
	sharedTask := NewTask(func(c *Context, _ None) (string, error) {
...
func BenchmarkTaskWithDependencies(b *testing.B) {
	baseTask := NewTask(func(c *Context, input int) (int, error) {
		return input * 2, nil
	})

	dependentTask := NewTask(func(c *Context, input int) (int, error) {
	...
	for _, numTasks := range []int{1, 2, 5, 10, 20, 50} {
		b.Run(fmt.Sprintf("tasks-%d", numTasks), func(b *testing.B) {
			...
			for i := 0; i < numTasks; i++ {
				taskID := i
				tasks[i] = NewTask(func(c *Context, input int) (int, error) {
