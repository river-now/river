...
`./control/wave.config.json`. Let's call this the "**Wave config file**".

The Wave config file utilizes a JSON schema to help make its surface area
discoverable and easy to use.

The default Wave config file generated by the River bootstrapper will look
something like this:
...
```json
{
	...
	"Core": {
		"ConfigLocation": "control/wave.config.json",
		"DevBuildHook": "go run ./control/cmd/build --dev --hook",
		"ProdBuildHook": "go run ./control/cmd/build --hook",
		"MainAppEntry": "control/cmd/serve",
		"DistDir": "control/dist",
		"StaticAssetDirs": {
			"Private": "assets/private",
			"Public": "assets/public"
		},
		"CSSEntryFiles": {
			"Critical": "frontend/styles/main.critical.css",
			"NonCritical": "frontend/styles/main.css"
		},
		"PublicPathPrefix": "/"
	},
	"River": {
		"UIVariant": "react", // or "preact" or "solid"
...
Most projects will never need to edit their Wave config file, but it's there if
you need it. In addition to supporting custom file names and locations to
override River conventions, you can also add any file or directory watching
hooks that you may need to granularly run your own build processes, process
templates, or whatever.

For example, for the site that serves this documentation, we use the following
watch configuration to tell the dev server to revalidate the page when we edit a
markdown file:
...
```json
{
	...
	"Watch": {
		...
		"Include": [
			{
				"Pattern": "assets/private/markdown/**/*.md",
...
You could also use it to run any random build command, such as compiling a Rust
crate to WASM and copying it into your static public assets folder:
...
```json
{
	...
	"Watch": {
		...
		"Include": [
			{
				"Pattern": "your-crate/src/**/*.rs",
				"OnChangeHooks": [
					{
						"Cmd": "make handle-rust-build"
					}
				],
				"RestartApp": true
			}
...
Wave has full support for monorepos; just be careful to set everything up
correctly in your config. Pay close attention to the instructions (accessible
via the JSON schema) for each field, as some fields expect their paths to be set
relative to the process `cwd`, while others expect their paths to be set
relative to a specific other directory (for example, your
`River.HTMLTemplateLocation` should be set relative to your
`Core.StaticAssetsDirs.Private` directory).
