...
func Core() *mux.Router {
	...
	mux.SetGlobalHTTPMiddleware(r, control.Wave.ServeStatic(true))
	...
	mux.SetGlobalHTTPMiddleware(r, control.Wave.FaviconRedirect())

	mux.RegisterHandler(r, "GET", "/*", control.River.GetLoadersHandler(LoadersRouter))

	for method := range supportedAPIMethods {
		mux.RegisterHandler(
			r, method, ActionsRouter.MountRoot("*"), control.River.GetActionsHandler(ActionsRouter),
		)
	}
